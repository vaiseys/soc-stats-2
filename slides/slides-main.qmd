...
# Causal inference: theoretical background
...
with the potential outcome variables because we can’t observe both $Y^0$ and 
...
- We think so much about omitted variable bias that we don’t often consider the 
...
Propensity scores are a solution to the problem of _sparseness_. If we can't
find exact matches for each case because of high dimensional (or fine-grained) 
data, can we reduce the complexity of the data and find "good enough" matches?
...
What are the propensity scores for subjects in different levels of 
_S_? 
...
Because exact matching is impossible when S comprises many variables, 
propensity scores allow us to summarize S in a single, continuous variable. 
This allows comparing "apples to apples" as long as we are comfortable with
"appleness" as defined by the propensity score.
...
- Then I will present some newer approaches that aim to rectify the weaknesses
of the traditional approach


## Traditional workflow

```{r workflow, out.width="70%"}
knitr::include_graphics(here("images", "workflow.JPG"))

```

::: aside

Adapted from Diamond and Sekhon 2014. S/W/M = stratify, weight, or match.
...
## Distribution of propensity score estimates
...
# density plot
ggplot(d, aes( x = ps1 , fill = smoker, color = smoker, after_stat(scaled) )) + 
  geom_density(alpha = .3) +
...
overall in two primary ways:
  - **Matching** (e.g., matching treated cases to controls with same or very 
  similar p-scores)
...
- Balance on the one-dimensional propensity score, however, does _not_
guarantee that the treatment and control groups will be balanced on each of the
individual component variables that was used to estimate the p-score. 
...
d %>% 
  arrange(ps1, mbsmoke) %>% 
  filter(mbsmoke==1 | dplyr::lead(mbsmoke)==1) %>%
  select(smoker:ps1) %>%
  head(n = 8)
```
...
control groups aren't systematic for any variable. We will talk about more formal assessments of balance soon.
...
- Propensity scores are a means to an end: making *all* the *S* variables independent of *T*.
...
- If this is the propensity-score equation, $x_1$ and $x_2$ have the same effect on the propensity. Thus *any* combination of $x_1$ and $x_2$ with the same *sum* will produce the same propensity score, even if the cases are quite different. We'll talk more about this soon.
...
There are some smokers with higher propensities than any nonsmoker and some nonsmokers with lower propensities than any smoker. We would need to decide if these violate the overlap assumption or whether these non-overlapping regions are strictly due to sampling error.
...
```{r strip_plot}
ggplot( d , 
        aes( x = smoker , y = ps1 , color = smoker )) +
  geom_jitter(alpha = .5, size = .3, width = .2) +
  theme(legend.position = "none") +
  coord_flip() +
  labs(x = "", y = "Est. propensity score") +
  annotate(geom="rect", ymin = .013, ymax = .039, 
           xmin = .8, xmax = 1.2, fill = "#F8766D", alpha = .2) +
  annotate(geom="rect", ymin = .849, ymax = .926, 
           xmin = 1.8, xmax = 2.2, fill = "#00BFC4", alpha = .2) +
  theme_light()

  
```

## Three options to start using p-scores

::: {.incremental}

1. **Stratify**: divide the cases into strata of the p-score (perhaps 5-10) to
approximate exact matching (a.k.a., "subclassification" or "interval matching")
2. **Match**: create (typically) 1:1 pairs of treatment and control cases
with propensity scores that are as similar as possible
3. **Weight**: use the p-score to make inverse probability of treatment weights
to balance the distributions of the T&C samples
...
- This approach does not sort individual cases into bins.
- It uses **inverse probability of treatment weights** (IPTW) based on each case's propensity score directly.
...
In surveys, we give more weight to groups that don’t show up as much 
as they should in the sample. The logic with IPTW is the same – in an 
...
2. A version that contains a regression specification

```{r, echo = TRUE}

# List version
trt_form1 <- "mbsmoke ~ mmarried + mrace + alcohol + fbaby + 
  mage + medu + nprenatal"
...
```{r weightcompare_boxplot}
# boxplot comparison
## unweighted
p1 <- ggplot(d, aes( y = pscore , x = smoker )) + 
  geom_boxplot(outlier.alpha = .3) +
  labs(y = "Estimated Propensity Score" ,
       x = "Original") +
  theme(legend.position = "top") +
...
## weighted
p2 <- ggplot(d, aes( y = pscore , x = smoker, weight = attwt )) + 
  geom_boxplot(outlier.alpha = .3) +
...
- `WeightIt` is an excellent package written by Noah Greifer
- Just like `MatchIt` brings many capabilities under one pacakge, `WeightIt` serves as a wrapper on the myriad weighting algorithms out there, giving them a unified syntax.
...
This `summary` will give you a sense of the distribution of weights. The 
lowest weight is `r round(min(W1$ps), 3)`. These are cases that are so
...
"Weights with low variability are desirable because they improve the precision 
of the estimator. This variability is presented in several ways: by the ratio 
of the largest weight to the smallest in each group, the coefficient of 
variation (standard deviation divided by the mean) of the weights in each 
group, and the effective sample size computed from the weights." (from
WeightIt help)
...
- This is like what psychologists call an **effect size**: the difference in z-scores
between the treatment and control groups.
- The convention is that this should be no greater than .1 for any T&C 
comparison for _any_ variable.
...
- We want to match the overall *shape* of the distribution, not just the mean.
- KSD is the proportion of non-overlap between two distributions (or maximum distance between two cumulative distributions).
- Two identical distributions have distance of 0; two non-overlapping distributions have distance of 1.

```{r, out.width="35%"}
knitr::include_graphics(here("images", "KS2_Example.png"))

```

## Balance checking with `cobalt`

- `cobalt` is an *excellent* package written by Noah Greifer
...
I don't want to type the same thing a million times so I define a custom function. 
...
```{r iptw_love, echo=TRUE, out.width="60%"}
love_plot(W1)
```
...
```{r balplot_iptw, echo = TRUE, out.width="65%"}
